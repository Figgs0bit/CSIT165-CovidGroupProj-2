...
#time_series_covid19_deaths_global.csv
PopDeaths_US <- as.data.frame(read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/refs/heads/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_US.csv"))

#Renames Deaths_US data frame to PopDeaths
deaths_US <- data.frame(PopDeaths_US)

#loads the population column from data frame Deaths_US
popCounties <- deaths_US[12]

#Population colum removed from Deaths_US dataframe
...
UScolmax <- ncol(confirmed_US[,]) #limit Qty of Cols 1154
...
summary_data <- confirmed %>%
  ...
  summarise(
    Total_Confirmed = sum(!!sym(latest_date), na.rm = TRUE),
...
# Categorize data into low, medium, and high based on quartiles
summary_data <- summary_data %>%
  mutate(
    Confirmed_Category = cut(
      Total_Confirmed,
      breaks = quantile(Total_Confirmed, probs = c(0, 0.25, 0.75, 1), na.rm = TRUE),
      labels = c("Low", "Medium", "High"),
      include.lowest = TRUE
    ),
    Deaths_Category = cut(
      Total_Deaths,
      breaks = quantile(Total_Deaths, probs = c(0, 0.25, 0.75, 1), na.rm = TRUE),
      labels = c("Low", "Medium", "High"),
      include.lowest = TRUE
    )
  )
...
map <- leaflet(summary_data) %>%
  ...
  addCircleMarkers(
    ...
    popup = ~paste0(
      ...
      "Total Confirmed: ", Total_Confirmed, "<br>",
      "Total Deaths: ", Total_Deaths
    ),
    color = ~confirmed_palette(Confirmed_Category),
    radius = ~sqrt(Total_Confirmed) / 10,
    group = "Confirmed"
  ) %>%
  addCircleMarkers(
    ~Mean_Long, ~Mean_Lat,
